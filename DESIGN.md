...
- **大留白**。每屏只放一个主角。
- **同一字体族**。强调靠重量 + 颜色 + marker 装饰，不切 serif。
...
**红线**：hero 标题 / 副标题 / accent 强调字 **不切 family**。要让某个词「跳出来」用重量 + 颜色 + marker 装饰，不是换字体。

### 3.2 字号阶梯（clamp 响应式）

| Token | clamp | 用在 |
|---|---|---|
| `text-display-xl` | `clamp(3rem, 7vw, 5rem)` | 首页 hero「一个空间。四个房间。」 |
| `text-display-lg` | `clamp(2.25rem, 5vw, 3.5rem)` | PageHeader hero 标题 |
| `text-display-md` | `clamp(1.75rem, 3.5vw, 2.5rem)` | 内页 section title |
...
```css
.accent          /* 行内强调 (em / span)：weight 800 + marker stroke */
.accent-display  /* hero 强调：weight 900 + 更显眼 marker + accent-shimmer 流光 */
.title-sub       /* 副标题专用：page accent 色 + 0.66 opacity，不带 marker */
```

`.accent-display` 自动跑 `accent-shimmer` 6s 流光（gradient 从中间扫过），呼应苹果发布会标题的金属光泽。
...
|---|---|
| `wrap` | `1120px` |
| `wrap-narrow` | `680px`（用在阅读视图，例如长文 article） |
...
| `halo-pulse` | 7s | 光晕慢扩张 |
| `accent-shimmer` | 6s | `.accent-display` 标题流光 |
...
- 标题 `.text-display-lg`，副标题用 `.title-sub` (accent 色 0.66 opacity)。
...
### 6.5 Marker stroke（accent 下划线）

`.accent` / `.accent-display` 文字下方有暖色记号笔扫过的效果，靠 `background-image` 渐变 + `background-position` 控制位置。颜色用 `var(--accent-stroke)` / `var(--accent-stroke-mid)`。
...
- ❌ subtitle 用 `text-ink-3` 灰 → ✅ `.title-sub` (page accent 色 0.66)
- ❌ subtitle 加 marker stroke → ✅ 装饰留给 `.accent-display`
- ❌ 数据散落在 JSX → ✅ 集中到 [`lib/content.ts`](./lib/content.ts)
- ❌ serif italic 做强调 → ✅ `.accent-display`（heavy + marker + shimmer）
